...
client.loopBlaze({
  async onMessage(msg) {
    ...
    if(msg.data === "?"|| msg.data === "？"){
      client.sendMessageText(UserId,"输入'/'会系统弹出6个单词进行造句。\n默认情况下不记忆前文语境，如果需要记忆前文语境，请在输入的字符末尾添加'+'号。\n")
    }
     else if(msg.data === "/"){
      randomWords = selectWords(6)
      client.sendMessageText(UserId,"请选择单词进行练习")
      ...
      client.sendAppButtonMsg(UserId, [
        ...
        {
          label: randomWords[1], // 按钮的文本内容
          color: '#FF0000', // 16进制按钮的颜色，如： #FF0000
          action: `input: ${randomWords[1]}`, // 按钮的跳转链接
        },
        {
          label: randomWords[2], // 按钮的文本内容
          color: '#FF0000', // 16进制按钮的颜色，如： #FF0000
          action: `input: ${randomWords[2]}`, // 按钮的跳转链接
        },
        {
          label: randomWords[3], // 按钮的文本内容
          color: '#FF0000', // 16进制按钮的颜色，如： #FF0000
          action: `input: ${randomWords[3]}`, // 按钮的跳转链接
        },
        {
          label: randomWords[4], // 按钮的文本内容
          color: '#FF0000', // 16进制按钮的颜色，如： #FF0000
          action: `input: ${randomWords[4]}`, // 按钮的跳转链接
        },
        {
          label: randomWords[5], // 按钮的文本内容
          color: '#FF0000', // 16进制按钮的颜色，如： #FF0000
          action: `input: ${randomWords[5]}`, // 按钮的跳转链接
        },
    ...
    let RawZhData = ""; //初始化的
      ...
      if (checkl === "chinese") {
        ...
        if (rawData.charAt(rawData.length -1) === "+"){RawEnData = PreviousQuestion+Previousanswer+RawEnData;//上次疑问+上次回答+本次输入=本次疑问,最后一位带有+号
        }
        console.log(`本次对话函数运行时GPT被问到的问题：${RawEnData}`);
      ...
      else if(msg.data === "?"||"？"||"/"){}//消除这些符号时的弹出：仅支持中英文消息
  ...
  messages: [
    {
      ...
      "I would like you to answer my questions based on facts and pay attention to the accuracy of the content of your answers. And ask me enlightening questions to facilitate our discussion. I want you to keep your reply neat, limiting the reply to 150 words.",
    },
    {
      role: "user",
      content: `${text}`,
